...
# FUNCTIONS
def set_timeind_slice(
    df, time_col, tz_to, tz_from="UTC", start="1970-01-01", end="2050-12-31"
):
    ...
    df = df.loc[
        pd.Timestamp(start, tz=tz_to) : pd.Timestamp(end, tz=tz_to)
    ].sort_index()
...
def get_count_perc(x, rm_zero, dropna):
    ...
    df_count_perc = pd.concat(
        [
            df_count,
            (x.value_counts(normalize=True, dropna=dropna) * 100).rename(
                df_count.name + "%"
            ),
        ],
        axis=1,
...
def value_counts_plus(df, rm_zero=False, dropna=False):
    return (
        pd.concat([get_count_perc(df[ii], rm_zero, dropna) for ii in df], axis=1)
        .fillna("")
        .astype(str)
    )
